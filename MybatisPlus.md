...
| AUTO        | 数据库ID自增，**依赖于数据库**。该类型请确保数据库设置了 ID自增 否则无效 |
| ----------- | ------------------------------------------------------------ |
| NONE        | 未设置主键类型。若在代码中没有手动设置主键，则会根据**主键的全局策略**自动生成（默认的主键全局策略是基于雪花算法的自增ID） |
| INPUT       | 需要手动设置主键，若不设置。插入操作生成SQL语句时，主键这一列的值会是`null`。 |
...
![image-20220428004136465](C:\Users\AUSA\AppData\Roaming\Typora\typora-user-images\image-20220428004136465.png)
